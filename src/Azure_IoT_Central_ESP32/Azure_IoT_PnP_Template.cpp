...
#define TELEMETRY_PROP_NAME_FLEXSENSORVALUE "FlexSensorValue"
...
static void get_flex_sensor_value(int32_t* flex_sensor_value) {
    ...
    size_t* payload_buffer_length)
{
  ...
  int32_t flex_sensor_value;

  // Acquiring sensor data
  get_flex_sensor_value(&flex_sensor_value);
  ...
  */
  ...
  rc = az_json_writer_append_property_name(
      &jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_FLEXSENSORVALUE));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding flex sensor value property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, flex_sensor_value);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding flex sensor value property value to telemetry payload.");
